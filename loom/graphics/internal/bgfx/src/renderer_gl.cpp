        ...
        void setRenderContextSize(uint32_t _width, uint32_t _height, uint32_t _msaa = 0, bool _vsync = false)
        {
            ...
            ||  _height != 0)
            {
                if (!m_glctx.isValid() )
                {
                    ...
                    // BJG - set the FBOs based on values passed from CCES2Renderer.mm
...
//      case GL_SAMPLER_2D_SHADOW:
    ...
    void Program::create(const Shader& _vsh, const Shader& _fsh)
    {
        ...
        if (!cached)
        {
            ...
            // LOOM: moved to AFTER the init call
        ...
        // LOOM: moved here as otherwise N7 bombs
        GL_CHECK(glDetachShader(m_id, _vsh.m_id) );
        GL_CHECK(glDetachShader(m_id, _fsh.m_id) );
...
#if !BGFX_CONFIG_RENDERER_OPENGL
        // it appears we need to swizzle the reload texture on ES2 devices?
        // Adding this to fix issue: https://theengineco.atlassian.net/browse/LOOM-1758
        // I added a comment there, we should fix swizzle for once and for all
        rgbaToBgra(_mem->data, _rect.m_width, _rect.m_height);        
#endif        
...
#   endif // BGFX_CONFIG_RENDERER_OPENGL >= 31
    ...
    void Context::rendererSubmit()
    {
        ...
        {
            GL_CHECK(glBindFramebuffer(GL_FRAMEBUFFER, s_renderCtx.m_msaaBackBufferFbo ? s_renderCtx.m_msaaBackBufferFbo : s_renderCtx.m_backBufferFbo) );
